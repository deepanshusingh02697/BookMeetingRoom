...
export default function AdminEquipment() {
  ...
  const [editEquip, setEditEquip] = useState<EquipType | null>(null);

  const { data, loading, error, refetch } =
    useQuery<GetEquipment_Interface>(GetEquipment_Query);

  const [createEquipment, { loading: creating }] =
    useMutation<CreateEquipment_Interface>(createEquipment_Mutation);

  const [editEquipment, { loading: editing }] =
    useMutation<udpateEquipment_Interface>(udpateEquipment);

  const equipments = data?.GetEquipments ?? [];

  const handleEdit = (eq: EquipType) => {
    setEditEquip(eq);
    setShowForm(true);
  };
  ...
  const handleSubmit = async (form: { name: string }) => {
    try {
      if (editEquip) {
        await editEquipment({
          variables: {
            equipmentId: Number(editEquip.id),
            name: form.name,
          },
        });
      } else {
